...
#import "FSBackgroundPromptView.h"
 ...
 */
+ (void)showMyWindowOnTarget:(PBaseViewController*)baseViewController withPopVc:(UIViewController*)poPBaseViewController
{
    //初始化一个自定义弹窗视图
    ...
    myWindow.hidden = NO;
    myWindow.rootViewController = poPBaseViewController;
    ...
    [poPBaseViewController.view addGestureRecognizer:[[UISwipeGestureRecognizer alloc] initWithTarget:baseViewController action:dismissPopview]];
...
void tapToHidePopViewAction(id sender, SEL _cmd)
{
    ...
    PBaseViewController* baseViewController = ((PBaseViewController*)sender);
    __weak typeof(baseViewController) weakSelf = baseViewController;
    [UIView animateWithDuration:0.5 animations:^{
...
+ (void)showSearchView:(PBaseViewController*)target andNaviBar:(PBaseNaviagtionBarView*)naviBar
{
...
+ (void)removeSearchVCFromCurrent:(PBaseViewController*)target
{
    [UIView animateWithDuration:0.4 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^
     {
...
+ (void)showLoginView:(PBaseViewController*)target andNaviBar:(PBaseNaviagtionBarView*)naviBar
{
...
+ (void)removeLoginViewFromCurrent:(PBaseViewController*)target
{
    [UIView animateWithDuration:0.4 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^
     {
         target.window.frame = CGRectMake(0, PJ_SCREEN_HEIGHT, PJ_SCREEN_WIDTH, PJ_SCREEN_HEIGHT);
...
+ (void)showShoppingCartView:(PBaseViewController*)target  andNaviBar:(PBaseNaviagtionBarView*)naviBar
{
//    //由storyboard根据LoginView获取到登录界面
...
//    [UIView animateWithDuration:0.4 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
//        target.window.frame = CGRectMake(0, 0, PJ_SCREEN_WIDTH, PJ_SCREEN_HEIGHT);
//        ((FourServicepingCartController*)shopping.toPBaseViewController).delegate = naviBar ? naviBar : target;
//    } completion:^(BOOL finished) {
//        [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleDefault animated:YES];
//    }];
}
...
{
//    CZJDetailViewController* detailView = (CZJDetailViewController*)[PUtils getViewControllerFromStoryboard:kCZJStoryBoardFileMain andVCName:kCZJStoryBoardIDGoodsDetailVC];
...
+ (void)showReloadAlertViewOnTarget:(UIView*)targetView withReloadHandle:(GeneralBlockHandler)reloadhandle
{
    if (!VIEWWITHTAG(targetView, 2626))
    {    LoadingFailedAlertView* loadingFaildView = [self getXibViewByName:@"LoadingFailedAlertView"];
...
+ (void)showNoDataAlertViewOnTarget:(UIView*)targetView withPromptString:(NSString*)promptStr
{
    ...
    {
        FSBackgroundPromptView* nodataAlertView = [self getXibViewByName:@"FSBackgroundPromptView"];
...
{
//    CZJStoreDetailController* storeDetail = (CZJStoreDetailController*)[PUtils getViewControllerFromStoryboard:kCZJStoryBoardFileMain andVCName:kCZJStoryBoardIDStoreDetailVC];
...
{
    FSBackgroundPromptView* promptView = [self getXibViewByName:@"FSBackgroundPromptView"];
